  ...
  <div class="confirmOrder">
    ...
    <div class="content">
        ...
        <div class="goods-list">
          <ul>
            <li v-for="item in getCheckGoods" :key="item.id">
              ...
              <span class="pro-price">{{item.specialPrice}}元 x {{item.count}}</span>
              <span class="pro-status"></span>
              <span class="pro-total">{{item.specialPrice * item.count}}元</span>
            </li>
          </ul>
        </div>
      </div>
      ...
      <div class="section-bar">
        <div class="btn">
          <router-link to="/shop/shoppingCart" class="btn-base btn-return">返回购物车</router-link>
...
export default {
  ...
  created() {
    ...
    if (this.getCheckNum < 1) {
      ...
      this.$router.push({ path: "/shop/shoppingCart" });
    }
  },
  computed: {
  ...
  methods: {
    ...
    addOrder() {
      ...
      for (let i = 0; i < this.getCheckGoods.length; i++) {
          let goods = this.getCheckGoods[i]
          let temp = {
      ...
      submit(params).then((res)=>{
          if(res.code == 200){
            ...
            for (let i = 0; i < cartIdList.length; i++) {
                // 删除已经结算的购物车商品
                // todo 后续优化
                // this.deleteShoppingCart(cartIdList.length[i]);
            }

             // 提示结算结果
            this.notifySucceed("订单提交成功!请尽快支付!");
            this.$router.push({ path: "/shop/order" });
            
          }else{
            this.notifySucceed("订单提交失败!请重试!");
          }
      })
...
.confirmOrder .content .section-count .money-box li {
  list-style:none;
}
